...
from src.ai.application.services.embedding_service import EmbeddingService
...
def test_get_embedding_model_loads_and_caches(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "src.ai.application.services.embedding_service.get_asset_manager_service",
        return_value=mock_asset_manager,
    )
    mock_fastembed_class = mocker.patch(
        "src.ai.application.services.embedding_service.TextEmbedding"
    )
